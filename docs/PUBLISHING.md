...
## 5b. iPhone (Safari / App Store) — 🔴 YOU
iPhone is a separate path (Xcode + Apple Developer account). **No Mac?** Run the
**safari-build** GitHub Actions workflow (Actions tab → Run workflow) — it builds the
iOS Xcode project on a cloud Mac and hands back a downloadable artifact. Then finish
signing + App Store submission per [`IOS_BUILD.md`](IOS_BUILD.md). On a Mac you can do
it all locally: `npm run build:safari`, then follow `IOS_BUILD.md` end-to-end.
